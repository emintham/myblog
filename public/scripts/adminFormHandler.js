...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!form) {
    ...
    console.warn(
      'AdminFormHandler: No form with class "post-form" found on this page.'
    );
    // }
    return;
  }
  ...
  function showFeedback(message, type) {
    if (feedbackMessage && feedbackDiv) {
      ...
      if (viewPostLink && type !== "success-message") {
        // Hide view link on error/warning
  ...
  function handlePostFormSuccess(event) {
    ...
    if (actionType === "create") {
      if (
        ...
        console.warn(
          "[AdminFormHandler] Create post success, but newSlug is missing or invalid for redirect. Result:",
          result
        );
        showFeedback(
      ...
      if (
        pageH1 &&
        result.newSlug &&
        form.dataset.originalSlug &&
        result.newSlug !== form.dataset.originalSlug
      ) {
      ...
      const originalSlugInput = form.querySelector(
        'input[name="originalSlug"]'
      );
      if (originalSlugInput && result.newSlug)
        originalSlugInput.value = result.newSlug;

      const originalFilePathInput = form.querySelector(
        'input[name="originalFilePath"]'
      );
      if (originalFilePathInput && result.newFilePath)
        originalFilePathInput.value = result.newFilePath;

      const originalExtensionInput = form.querySelector(
        'input[name="originalExtension"]'
      );
      if (originalExtensionInput && result.newExtension)
        originalExtensionInput.value = result.newExtension;
    }
  }
