...
postType: standard
draft: false
lastEdited: 2025-11-22T18:03:40.703Z
description: >-
...
If you don't game much, it might surprise you that the PS4 controller's joystick is sensitive around the middle and less so around the outside. As such, a linear map of the joystick value to motor movement results in a jittery motor movement at rest. A simple solution is to define a "dead zone" around a small area of the middle that maps to 0 to work around this sensitivity.

Secondly, there's a minimum amount of drive you need to provide your motor in order for it to overcome its inertia. When mapping joystick value to motor value, the minimum drive should start at your motor's threshold.
...
After you're happy with your code, you'll want to ensure that your controller is able to automatically pair with the RPi across restarts and that your script for running the car starts upon boot up. To do the latter, you'll need to set up a systemd service.
